...
export class TsMongooseVisitor extends BaseVisitor<TypeScriptMongoPluginConfig, TypeScriptMongoPluginParsedConfig> {
  constructor(private _schema: GraphQLSchema, pluginConfig: TypeScriptMongoPluginConfig) {
    super(pluginConfig, {
      ...
      enumsAsString: getConfigValue<boolean>(pluginConfig.enumsAsString || true, true),
      avoidOptionals: getConfigValue<boolean>(pluginConfig.avoidOptionals || false, false),
  ...
  private _resolveDirectiveValue<T>(valueNode: ValueNode): T | null {
  ...
  private _getDirectiveArgValue<T>(node: DirectiveNode, argName: string): T | null {
  ...
  private _buildInterfaces(interfaces: ReadonlyArray<NamedTypeNode>): (string | null)[] {
    return (interfaces || [])
      .map(namedType => {
        ...
        const { astNode } = schemaType;

        if (!astNode) {
          return null;
        }
        const abstractEntityDirective = this._getDirectiveFromAstNode(astNode, Directives.ABSTRACT_ENTITY);
  ...
  private _handleIdField(fieldNode: FieldDefinitionNode, tree: FieldsTree, addOptionalSign: boolean): void {
    tree.addField(
      ...
      wrapTypeNodeWithModifiers('Foo', fieldNode.type)
    );
  }
  ...
  private _buildFieldsTree(fields: ReadonlyArray<FieldDefinitionNode>): FieldsTree {
    ...
    fields.forEach(field => {
      ...
      const mapPath = mapDirective ? this._getDirectiveArgValue<string>(mapDirective, 'path') : null;
  ...
  InterfaceTypeDefinition(node: InterfaceTypeDefinitionNode): string | null {
    ...
    if (!node.fields || !discriminatorField) return null;
  ...
  UnionTypeDefinition(node: UnionTypeDefinitionNode): string | null {
    ...
    if (!node.types) return null;
    const possibleTypes = node.types
      .map(namedType => {
        const schemaType = this._schema.getType(namedType.name.value);
        if (!schemaType) return null;
        const { astNode } = schemaType;
        const entityDirective = this._getDirectiveFromAstNode(astNode as Directivable, Directives.ENTITY);
        const abstractEntityDirective = this._getDirectiveFromAstNode(
          astNode as Directivable,
  ...
  ObjectTypeDefinition(node: ObjectTypeDefinitionNode): string {
    ...
    if (!node.interfaces || !node.fields) return '';
    ...
    console.log(implementingInterfaces);

    const name = this.convertName(node, { suffix: this.config.dbTypeSuffix });

    const clazz = new DeclarationBlock(this._declarationBlockConfig)
      .withDecorator('@Schema()')
      .export()
      .asKind('class')
      .withName(name)
    ...
    const docType = new DeclarationBlock(this._declarationBlockConfig)
      .export()
      .asKind('type')
      .withName(this.convertName(name, { suffix: 'Document' }))
      .withContent(`HydratedDocument<${name}>`).string;

    const schema = new DeclarationBlock(this._declarationBlockConfig)
      .export()
      .asKind('const')
      .withName(this.convertName(name, { suffix: 'Schema' }))
      .withContent(`SchemaFactory.createForClass(${name})`).string;

    return [clazz, docType, schema].join('\n');
  }
}
